import React, { useEffect, useState } from "react"; // Added React import for Fragments
...
function AdminProducts() {
  ...
  const filteredProducts = sortedProducts.filter(product => {
    if (searchTerm) {
      ...
      // Added optional chaining (?.) to prevent errors on null values
      const matchesSearch =
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row md:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            >
              {key === "in-stock" && filter === key && <CheckCircle size={14} />}
              {key === "out-of-stock" && filter === key && <XCircle size={14} />}
              {key === "low-stock" && filter === key && <AlertCircle size={14} />}
              {label}
            </button>
          ))}
        </div>
                ...
                <thead className="sticky top-0 z-20 bg-gradient-to-r from-emerald-50 to-white backdrop-blur-sm">
                  <tr>
                    ...
                    <th className="p-4 text-left font-semibold text-gray-700 border-b cursor-pointer" onClick={() => handleSort('stockQuantity')}>
                      <div className="flex items-center gap-1">
                        Stock
                        {sortConfig.key === 'stockQuantity' && (sortConfig.direction === 'asc' ? <ChevronUp size={14} /> : <ChevronDown size={14} />)}
                      </div>
                    </th>
                    ...
                    <tr className="border-t hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-1 font-bold text-gray-800">
                          <IndianRupee size={14} /> {product.price.toLocaleString('en-IN')}
                        </div>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className={`text-sm font-medium ${product.stockQuantity < 5 ? 'text-red-600' : product.stockQuantity < 10 ? 'text-orange-600' : 'text-emerald-600'}`}>
                                {product.stockQuantity}
                              </span>
                              <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full ${product.stockQuantity < 5 ? 'bg-red-500' : product.stockQuantity < 10 ? 'bg-orange-500' : 'bg-emerald-500'}`}
                                  style={{ width: `${Math.min((product.stockQuantity / 50) * 100, 100)}%` }}
                                ></div>
                              </div>
                            </div>
                            {product.stockQuantity < 10 && <p className="text-xs text-red-500 mt-1">Low stock</p>}
                          </div>
                        </div>
                      </td>
                      <td className="p-4">
                        <button
                          ...
                          className={`relative inline-flex items-center h-7 w-14 rounded-full transition-all duration-300 ${product.inStock ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-gray-300 hover:bg-gray-400'}`}
                        >
                          <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-lg transition-all duration-300 ${product.inStock ? "translate-x-8" : "translate-x-1"}`} />
                        </button>
                        <p className="text-xs mt-1 text-gray-500">{product.inStock ? 'Active' : 'Disabled'}</p>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <Link to={`/admin/products/${product._id}/edit`} className="p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                            <Edit size={16} />
                          </Link>
                          <button onClick={() => deleteProduct(product._id)} className="p-2 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
                        ...
                        <td colSpan="6" className="p-4 border-t">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            <div>
                              ...
                              <code className="text-xs bg-gray-100 px-2 py-1 rounded font-mono">{product._id}</code>
                            </div>
                            <div>
                              <p className="text-xs font-semibold text-gray-500 mb-2">Created</p>
                              <p className="text-sm">{new Date(product.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                            </div>
                            <div>
                              <p className="text-xs font-semibold text-gray-500 mb-2">Last Updated</p>
                              <p className="text-sm">{new Date(product.updatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                            </div>
              ...
              <div key={product._id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-sm font-medium px-2 py-1 rounded-full ${product.stockQuantity < 5 ? 'bg-red-100 text-red-700' : product.stockQuantity < 10 ? 'bg-orange-100 text-orange-700' : 'bg-emerald-100 text-emerald-700'}`}>
                          Stock: {product.stockQuantity}
                        </span>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => toggleRowExpand(product._id)} className="text-gray-400 hover:text-gray-600">
                    {expandedRows[product._id] ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                  </button>
                </div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500">{product.inStock ? 'Active' : 'Disabled'}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Link to={`/admin/products/${product._id}/edit`} className="flex-1 py-2 bg-blue-100 text-blue-700 rounded-lg text-sm flex items-center justify-center gap-1 hover:bg-blue-200">
                    <Edit size={14} /> Edit
                  </Link>
                  <button onClick={() => deleteProduct(product._id)} className="flex-1 py-2 bg-red-100 text-red-700 rounded-lg text-sm flex items-center justify-center gap-1 hover:bg-red-200">
                  ...
                  <div className="mt-4 pt-4 border-t space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-xs font-mono bg-gray-100 p-2 rounded">{product._id.slice(-8)}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-gray-500 mb-1">Created</p>
                        <p className="text-xs">{new Date(product.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-gray-500 mb-1">Updated</p>
                        <p className="text-xs">{new Date(product.updatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                      </div>
                    </div>
                  </div>
                )}
